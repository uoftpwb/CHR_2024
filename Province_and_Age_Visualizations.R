...
y_fill <- rep(c(background_colour, accent_background_colour), length.out = length(y_breaks) - 1)
...
age_group_ls_plot <- ggplot(canadaXage_avg_ls, aes(x = display_year, y = average_ls, group = age_ranges, color = age_ranges)) +
  ...
  scale_x_continuous(breaks = seq(min(canadaXage_avg_ls$display_year, na.rm = TRUE), max(canadaXage_avg_ls$display_year, na.rm = TRUE), by = 1)) +
  scale_color_brewer(palette = "Set2", type = "qual") +
  scale_fill_manual(values = y_fill)
...
age_group_ls_plot_CSS <- ggplot(canadaXageCSS_avg_ls, aes(x = year, y = average_ls, group = age_ranges, color = age_ranges)) +
  geom_rect(data = rect_data, 
            aes(xmin = -Inf, xmax = Inf, ymin = ymin, ymax = ymax, fill = fill), 
            color = NA, inherit.aes = FALSE, show.legend = FALSE) +
  geom_line() +
...
# Save the plot
ggsave("Output/Plots/trajectory_by_age_group_Canada_CSS.png", plot = age_group_ls_plot_CSS, width = 9, height = 3, dpi = 300)
ggsave("Output/Plots/trajectory_by_age_group_Canada_CSS.jpg", plot = age_group_ls_plot_CSS, width = 9, height = 3, dpi = 300)
ggsave("Output/Plots/trajectory_by_age_group_Canada_CSS.svg", plot = age_group_ls_plot_CSS, width = 9, height = 3, dpi = 300)
...
combined_data <- canadaXage_avg_ls %>%
  mutate(year = display_year) %>%
  full_join(canadaXageCSS_avg_ls, by = c("year", "age_ranges")) %>%
  full_join(gallup, by = c("year", "age_ranges")) %>%
...
age_group_ls_plot_combined <- ggplot() +
  geom_rect(data = rect_data, 
            aes(xmin = -Inf, xmax = Inf, ymin = ymin, ymax = ymax), 
            fill = accent_background_colour, color = NA, inherit.aes = FALSE, show.legend = FALSE) +
  geom_line(data = CCHS_data_filtered, aes(x = year, y = average_ls_CCHS, group = age_ranges, color = age_ranges, linetype = "CCHS")) +
  geom_line(data = CSS_data_filtered, aes(x = year, y = average_ls_CSS, group = age_ranges, color = age_ranges, linetype = "CSS")) +
  geom_line(data = Gallup_data_filtered, aes(x = year, y = average_ls_Gallup, group = age_ranges, color = age_ranges, linetype = "Gallup")) +
  labs(title = "Canadian Life Satisfaction by Age",
       y = "Average Life Satisfaction",
       color = "Age Groups",
...
print(age_group_ls_plot_combined)

ggsave("Output/Plots/trajectory_by_age_group_Canada_combined.png", plot = age_group_ls_plot_combined + theme(plot.background = element_rect(fill = "transparent", color = NA)), width = 9, height = 3, dpi = 300, bg = "transparent")
ggsave("Output/Plots/trajectory_by_age_group_Canada_combined.svg", plot = age_group_ls_plot_combined + theme(plot.background = element_rect(fill = "transparent", color = NA)), width = 9, height = 3, dpi = 300, bg = "transparent")
ggsave("Output/Plots/trajectory_by_age_group_Canada_combined.jpg", plot = age_group_ls_plot_combined, width = 9, height = 3, dpi = 300)
